...
bool Device::create() {
  ...
  if (HSA_STATUS_SUCCESS !=
    ...
    LogPrintfError("Unable to get physical execution ISA for HSA device %s",
                   agent_name);
    return false;
  }
  ...
  if (HSA_STATUS_SUCCESS !=
    ...
    LogPrintfError("Unable to get physical execution ISA name length for %s",
                   agent_name);
    return false;
  }
  ...
  if (HSA_STATUS_SUCCESS !=
    ...
    LogPrintfError("Unable to get physical execution ISA name for %s",
                   agent_name);
    return false;
  }
  ...
  if (execution_isa == nullptr || !execution_isa->runtimeRocSupported()) {
    LogPrintfError("Unsupported physical execution ISA %s for HSA device %s",
                   execution_isa_name.data(), agent_name);
    return false;
  }
  ...
  settings_ = hsaSettings;
  if (!hsaSettings ||
      !hsaSettings->create((agent_profile_ == HSA_PROFILE_FULL), *execution_isa,
                           execution_isa->xnack() == amd::Isa::Feature::Enabled,
                           coop_groups, isXgmi_)) {
    LogPrintfError("Unable to create settings for HSA device %s (PCI ID %x)", agent_name,
                   pciDeviceId_);
    return false;
  }
  if (isa != execution_isa) {
    // Hardware policy follows the execution ISA, but code generated by CLR
    // must use the wave mode of the ISA exposed for code-object selection.
    hsaSettings->lcWavefrontSize64_ = isa->versionMajor() < 10;
  }
