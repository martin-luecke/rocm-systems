...
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx942 --enable-writelane-rewrite --emit-ir=writelane_divergent_rewrite_kernel 2>/dev/null | %FileCheck %s --check-prefix=REWRITE
;
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=writelane_divergent_rewrite_kernel 2>/dev/null | %FileCheck %s --check-prefix=UNCHANGED
;
