...
; RUN:   && %not %raise_cli %t.hsaco --target-isa=gfx942 --enable-writelane-rewrite --emit-ir=writelane_sgpr_forced_use_kernel 2>&1 | %FileCheck %s --check-prefix=REFUSE
;
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=writelane_sgpr_forced_use_kernel 2>/dev/null | %FileCheck %s --check-prefix=UNCHANGED
;
